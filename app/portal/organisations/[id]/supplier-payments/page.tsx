...
export default async function SupplierPaymentsPage({
  ...
  const { data: payments } = await supabase
    ...
    .select(
      "id, payment_number, supplier_id, purchase_bill_id, payment_date, currency_code, amount_paid, bank_charges, total_cash_outflow, payment_method, reference_number, status, created_at"
    )
  ...
  const billMap = new Map(billRows.map((bill) => [bill.id, bill.bill_number]));
  ...
  const totalBankCharges = paymentRows.reduce(
    (sum, payment) => sum + Number(payment.bank_charges || 0),
    0
  );

  const totalCashOutflow = paymentRows.reduce(
    (sum, payment) => sum + Number(payment.total_cash_outflow || 0),
    0
  );

  const draftCount = paymentRows.filter(
    (payment) => (payment.status || "DRAFT") === "DRAFT"
  ).length;
    ...
    <main className="min-h-screen bg-[#F8FAFC]">
            ...
            <div className="flex items-start gap-5">
              ...
              <div>
                ...
                <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">
                  Review draft and recorded supplier payments for{" "}
                  {organisationName}. These support payables settlement,
                  supplier statements, bank reconciliation, expense analysis,
                  management reporting, and later posting workflows.
                </p>
              </div>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="grid gap-5 md:grid-cols-4">
          <div className="rounded-[1.5rem] border border-[#D9E3F4] bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              All supplier payments recorded.
            </p>
          </div>
          ...
          <div className="rounded-[1.5rem] border border-[#D9E3F4] bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Supplier payments before bank charges.
            </p>
          </div>
          ...
          <div className="rounded-[1.5rem] border border-[#D9E3F4] bg-white p-6 shadow-sm">
            ...
            <div className="mt-3 text-3xl font-semibold text-slate-950">
              {formatMoney(
                organisation.base_currency_code,
                Number(totalCashOutflow.toFixed(2))
              )}
            </div>
            <p className="mt-2 text-sm text-slate-500">
          ...
          <div className="flex flex-col gap-4 border-b border-[#D9E3F4] px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                A working register of supplier payments created for this
                organisation.
              </p>
            </div>
          ...
          {paymentRows.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-500">
                Create the first draft supplier payment for this organisation.
                    ...
                    <tr key={payment.id} className="hover:bg-[#F8FAFC]">
                      <td className="whitespace-nowrap px-6 py-5">
                        ...
                        <div className="mt-1 text-xs text-slate-500">
                          Ref: {payment.reference_number || "Not provided"}
                        </div>
                        <div className="mt-1 text-xs text-slate-500">
                      ...
                      <td className="whitespace-nowrap px-6 py-5 text-sm text-slate-600">
                        {supplierMap.get(payment.supplier_id || "") ||
                          "No supplier"}
                      </td>

                      <td className="whitespace-nowrap px-6 py-5 text-sm text-slate-600">
                        {billMap.get(payment.purchase_bill_id || "") ||
                          "Not linked"}
                      </td>
        ...
        <div className="mt-6 rounded-[1.5rem] border border-[#D9E3F4] bg-white p-6 text-sm leading-7 text-slate-600 shadow-sm">
          ...
          These supplier payments are draft operational records. They do not
          affect the ledger, payables, supplier statements, or bank balances
